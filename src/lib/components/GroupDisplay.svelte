...
<div class="w-full max-w-lg mx-auto">
					...
					{#each group.members.sort((a, b) => {
						...
						<li class="flex items-center gap-2">
							<div class="flex items-center gap-2">
								<span
									class={`font-mono font-bold ${member.gender === 'M' ? 'text-blue-700 dark:text-blue-500' : 'text-fuchsia-500'}`}
								>
									{member.gender}
								</span>
								<span
									class={`font-medium whitespace-nowrap ${member.isLeader ? 'text-green-600 dark:text-green-300' : ''}`}
									>{member.name}</span
								>
							</div>
						</li>
	...
	{#if (!$groups || $groups.length === 0) && !$isRegenerating}
		<div class="text-center text-gray-500 py-8">
			...
			{:else if $groupSettings.requireLeader && !$people
