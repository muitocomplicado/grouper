<script lang="ts">
	...
	function getNextFamilyNumber(currentPeople: Person[]): number {
		const usedNumbers = currentPeople.map((p) => p.familyNumber || 0).filter((n) => n > 0);
	...
	$: {
		...
		hasDuplicate =
			trimmedName !== '' &&
			[...$people].some(
				(p) => p.name.toLowerCase() === trimmedName.toLowerCase() && p.id !== editingPerson?.id
			);
				...
				p.map((person) =>
					person.id === editingPerson?.id
						? {
								...
								gender,
								familyNumber:
									familyNumber && parseInt(familyNumber) > 0 ? parseInt(familyNumber) : undefined,
								isLeader
							}
						: person
				)
